...
class TaskType(object):
    ...
    def supported_languages(self):
        """The programming languages this TaskType is able to handle.

        return ([string]): a list of programming language shorthand
            codes, i.e. some of the cms.LANG_* constants.

        """
        raise NotImplementedError("Please subclass this class.")

    @property
    def user_file_format(self):
        """The format we expect for user-provided files.

        return ([UserFileSchema]): a list of file schemas, one for each
            file we expect the contestant to provide.
    ...
    def dataset_file_format(self):
        """The format we expect for per-dataset admin-provided files.

        return ([DatasetFileSchema]): a list of file schemas, one for
            each file we expect the admin to provide.

        """
        raise NotImplementedError("Please subclass this class.")

    @property
    def compilation_file_format(self):
        """The format we expect for per-dataset autogenerated files.

        These files are generated by a compilation step.

        return ([CompilationFileSchema]): a list of file schemas, one
            for each file we expect to be generated.

        """
        raise NotImplementedError("Please subclass this class.")

    @property
    def testcase_file_format(self):
        """The format we expect for per-testcase admin-provided files.

        return ([TestcaseFileSchema]): a list of file schemas, one for
            each file we expect the admin to provide.
    ...
    @property
    def execution_file_format(self):
        """The format we expect for per-testcase autogenerated files.

        These files are generated by a execution/evaluation step.

        return ([ExecutionFileSchema]): a list of file schemas, one for
            each file we expect to be generated.
